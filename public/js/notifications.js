...
async function markAllNotificationsRead() {
  ...
  try {
    ...
    if (response.ok) {
      // Update semua notifikasi di frontend
      notifications = notifications.map(n => ({ ...n, isRead: true }));
      unreadCount = 0;
      
      // Render ulang dan update badge
      renderNotifications();
      updateNotificationBadge();
      ...
      // Opsional: Load ulang notifikasi dari server untuk memastikan
      setTimeout(() => {
        loadNotifications();
      }, 500);
    }
